...
## Phase 5: LLM 分析（文档指南方式）

**目标**: 提供 LLM 安全检查指南，让用户使用自己的 LLM 工具进行深度分析

**设计理念**: 与其在代码中集成 LLM API，不如提供结构化的检查提示词，让用户灵活使用任何 LLM。

### 优势

- 无需 API 密钥管理
- 用户可使用任何 LLM（Claude、ChatGPT、本地模型等）
- 更易维护（只需维护文档）
- 成本透明（用户自己控制）
...
- [x] 5.1 创建 LLM 安全检查指南
  - [x] 创建 `docs/llm-security-guide.md`
  - [x] 定义检查清单（命令注入、数据外泄、文件破坏等）
  - [x] 设计输出格式模板
  - [x] 添加使用示例

- [x] 5.2 添加导出功能
  - [x] 在 scanner.py 添加 `--export-for-llm` 参数
  - [x] 实现 skill 内容导出功能
  - [x] 输出格式化的 skill 信息和脚本内容
...
# 将输出内容 + docs/llm-security-guide.md 中的提示词发送给 LLM
```

### 验证标准

- [x] --export-for-llm 正确导出 skill 内容
- [x] 检查指南文档完整清晰
- [x] 示例输出格式正确
...
- [x] 6.1 添加命令行参数
  - [x] 在 `src/scanner.py` 添加 argparse
  - [x] 添加 `--mode` 参数（fast/standard/deep）
  - [x] 添加 `--export-for-llm` 参数
  - [x] 添加 `--no-ast` 参数
  - [x] 添加 `--help` 帮助信息
...
- [x] 6.3 更新文档
  ...
  - [x] 更新 `CLAUDE.md` 添加架构说明
  - [x] 更新 `docs/README.md` 添加 AST 规则说明
  - [x] 添加使用示例
...
- [x] 1.5 创建配置管理 ✅
- [x] 1.6 重构现有规则 ✅
- [x] 1.7 创建 RegexAnalyzer ✅

**Phase 2: Python AST 分析** - ✅ 已完成

- [x] 2.1-2.7 全部完成 ✅

**Phase 3: Shell AST 分析** - ✅ 已完成

- [x] 3.1-3.4 全部完成 ✅

**Phase 4: JavaScript AST 分析** - ✅ 已完成

- [x] 4.1 实现 JavaScript AST 解析器 ✅
- [x] 4.2 实现 JavaScript AST 规则 ✅
- [x] 4.3 添加测试用例 ✅

**Phase 5: LLM 分析（文档指南方式）** - ✅ 已完成

- [x] 5.1 创建 LLM 安全检查指南 ✅
- [x] 5.2 添加导出功能 ✅

**Phase 6: 命令行接口和文档** - ✅ 核心完成
...
**最后更新**: 2026-01-14
**当前阶段**: Phase 6 核心完成
**完成度**: 95% (Phase 1-6 核心完成，6.4/6.5 可选)
